...
static Node<RTOS::Timer::SoftwareTimer> *sTimerList = nullptr;
...
void RTOS::Semaphore::signal(void)
{
    value++;
}
...
size_t RTOS::Config::getAllocatedMemory(void)
{
    return mem.getAllocatedMemory();
}

size_t RTOS::Config::getFreeMemory(void)
{
...
RTOS::Mutex::Mutex(void) : flag(ATOMIC_FLAG_INIT)
{
}
...
void RTOS::Mutex::lock(void)
{
    while (flag.test_and_set(std::memory_order_acquire));
}

void RTOS::Mutex::unlock(void)
{
...
void TimerISR(void *)
{
    while(1)
    {
        Node<RTOS::Timer::SoftwareTimer> *tmp = sTimerList;
        while (tmp != nullptr)
        {
...
RTOS::Queue::~Queue(void)
{
    mem.deallocate(mQueue);
    mQueue = nullptr;
}

RTOS::Result RTOS::Queue::Send(void* item, uint32_t timeout)
{
    RTOS::Result result = RTOS::Result::RESULT_SUCCESS;
...
RTOS::Result RTOS::Queue::Receive(void* item, uint32_t timeout)
{
    RTOS::Result result = RTOS::Result::RESULT_SUCCESS;
    uint32_t start_time = tickCount;
...
RTOS::CircularBuffer::CircularBuffer(uint32_t mBuffer_size)
    : mHead(0u), mTail(0u), mCurrentSize(0u), mBufferSize(mBuffer_size)
{
}

RTOS::CircularBuffer::CircularBuffer(const CircularBuffer& old)
{
    mHead = old.mHead;
    mTail = old.mTail;
...
RTOS::CircularBuffer::~CircularBuffer(void)
{
    mem.deallocate(mBuffer);
}

RTOS::Result RTOS::CircularBuffer::init(void)
{
    if (mBufferSize == 0)
    {
        return RTOS::Result::RESULT_BAD_PARAMETER;
...
RTOS::Result RTOS::CircularBuffer::put(const uint8_t* data, uint32_t size, uint32_t timeout_ms)
{
    RTOS::Result result = RTOS::Result::RESULT_SUCCESS;
    uint32_t start_time = tickCount;
    while (mCurrentSize + size > mBufferSize)
    {
        ...
        {
            result = RTOS::Result::RESULT_CIRCULAR_BUFFER_TIMEOUT;
            return result;
        }
        else
        {
...
RTOS::Result RTOS::CircularBuffer::get(uint8_t* data, uint32_t size, uint32_t timeout_ms)
{
    RTOS::Result result = RTOS::Result::RESULT_SUCCESS;
    uint32_t start_time = tickCount;
